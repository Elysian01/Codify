...
for key, value in model_dict.items():
    print(f"{value} Model ")
    print(
        f"Classification Report:{classification_report(y_test, models[i].predict(X_test))}")
    print(
        f"{value} model accuracy: {round(accuracy_score(y_test, models[i].predict(X_test)), 2)}")
    print(f'{"-"*55}')
    i += 1
